...
//Initialization Function - boots up program
  function init() {
    updateDisplay ("To begin your trek at Philmont Scout Ranch, click any button!");
...
// Click event for each button codes for 1 column of events in map matrix
// i.e. North = 1st column; East = 2nd column; South = 3rd column; West = 4th column
    function btnNorth(){
      currentLvl = map[currentLvl][0];
      trail();
    }

    function btnEast(){
      currentLvl = map[currentLvl][1];
      trail();
    }

    function btnSouth(){
      currentLvl = map[currentLvl][2];
      trail();
    }

    function btnWest(){
      currentLvl = map[currentLvl][3];
      trail();
    }
